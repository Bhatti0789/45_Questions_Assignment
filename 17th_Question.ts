/* Shrinking Guest List: You just found out that your new dinner table won’t arrive in time for the dinner, and you 
   have space for only two guests.
    ...
    let guest = [ 'Bilal', 'Ali', 'Daniyal', 'Moazzam', 'Usman', 'Ayan' ]
    
        while ( guest.length > 2 )
        {
            const remGuest = guest.pop()

                console.log("Sorry" , remGuest , ", Due to limited space, only 2 people can be invited for Dinner")
        }
    
        guest.forEach((guest) => console.log( "Dear" , guest , "you are invited on Dinner") )